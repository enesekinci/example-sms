...
final class ResponseMessage
{
    ...
    public static function errorMessage(int $code): string
    {
        return match ($code) {
            ...
            1090 => 'This field is not last children',
    ...
    public static function errorMessages(): array
    {
        ...
        $keys = array_keys((new ReflectionClass(self::class))->getConstants());

        foreach ($keys as $key) {
            $messages[(new ReflectionClass(self::class))->getConstant($key)] = self::errorMessage((new ReflectionClass(self::class))->getConstant($key));
        }
